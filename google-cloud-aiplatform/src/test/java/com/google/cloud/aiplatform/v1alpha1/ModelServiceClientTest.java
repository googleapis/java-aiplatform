...
public class ModelServiceClientTest {
  ...
  @Test
  @SuppressWarnings("all")
  public void exportEvaluatedDataItemsTest() throws Exception {
    ExportEvaluatedDataItemsOperationResponse expectedResponse =
        ExportEvaluatedDataItemsOperationResponse.newBuilder().build();
    Operation resultOperation =
        Operation.newBuilder()
            .setName("exportEvaluatedDataItemsTest")
            .setDone(true)
            .setResponse(Any.pack(expectedResponse))
            .build();
    mockModelService.addResponse(resultOperation);

    ModelEvaluationName modelEvaluation =
        ModelEvaluationName.of("[PROJECT]", "[LOCATION]", "[MODEL]", "[EVALUATION]");

    ExportEvaluatedDataItemsOperationResponse actualResponse =
        client.exportEvaluatedDataItemsAsync(modelEvaluation).get();
    Assert.assertEquals(expectedResponse, actualResponse);

    List<AbstractMessage> actualRequests = mockModelService.getRequests();
    Assert.assertEquals(1, actualRequests.size());
    ExportEvaluatedDataItemsRequest actualRequest =
        (ExportEvaluatedDataItemsRequest) actualRequests.get(0);

    Assert.assertEquals(
        modelEvaluation, ModelEvaluationName.parse(actualRequest.getModelEvaluation()));
    Assert.assertTrue(
        channelProvider.isHeaderSent(
            ApiClientHeaderProvider.getDefaultApiClientHeaderKey(),
            GaxGrpcProperties.getDefaultApiClientHeaderPattern()));
  ...
  @Test
  @SuppressWarnings("all")
  public void exportEvaluatedDataItemsExceptionTest() throws Exception {
    StatusRuntimeException exception = new StatusRuntimeException(Status.INVALID_ARGUMENT);
    mockModelService.addException(exception);
